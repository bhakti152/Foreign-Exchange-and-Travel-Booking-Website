...
public partial class TravelPages_Cruise : System.Web.UI.Page
{
    ...
    private void BindGrid()
    {
        ...
        using (MySqlConnection con = new MySqlConnection(constr))
        {
            ...
            {
                cmd.CommandText = "SELECT * FROM tbl_details where type='domestic' and category='Cruise'";
                cmd.Connection = con;
    ...
    private void BindGrid1()
    {
        ...
        using (MySqlConnection con = new MySqlConnection(constr))
        {
            using (MySqlCommand cmd = new MySqlCommand())
            {
                cmd.CommandText = "SELECT * FROM tbl_details where offers='Exclusive' and category='Cruise' ";
                cmd.Connection = con;
                using (MySqlDataAdapter sda = new MySqlDataAdapter(cmd))
                {
                    DataTable dt = new DataTable();
    ...
    private void BindGrid2()
    {
        ...
        using (MySqlConnection con = new MySqlConnection(constr))
        {
            using (MySqlCommand cmd = new MySqlCommand())
            {
                cmd.CommandText = "SELECT * FROM tbl_details where type='international'  and category='Cruise' ";
                cmd.Connection = con;
                using (MySqlDataAdapter sda = new MySqlDataAdapter(cmd))
                {
                    DataTable dt = new DataTable();
