...
public partial class TravelPages_InternationalHoliday : System.Web.UI.Page
{
    ...
    private void BindGrid()
    {
        ...
        using (MySqlConnection con = new MySqlConnection(constr))
        {
            ...
            {
                cmd.CommandText = "SELECT * FROM tbl_details where type='international' and category='Holidays'";
                cmd.Connection = con;
    ...
    private void BindGrid1()
    {
        ...
        using (MySqlConnection con = new MySqlConnection(constr))
        {
            using (MySqlCommand cmd = new MySqlCommand())
            {
                cmd.CommandText = "SELECT * FROM tbl_details where type='international' and offers='Exclusive' and category='Holidays' ";
                cmd.Connection = con;
                using (MySqlDataAdapter sda = new MySqlDataAdapter(cmd))
                {
                    DataTable dt = new DataTable();
    ...
    private void BindGrid2()
    {
        ...
        using (MySqlConnection con = new MySqlConnection(constr))
        {
            using (MySqlCommand cmd = new MySqlCommand())
            {
                cmd.CommandText = "SELECT * FROM tbl_details where type='international'  and category='HoneyMoon' ";
                cmd.Connection = con;
                using (MySqlDataAdapter sda = new MySqlDataAdapter(cmd))
                {
                    DataTable dt = new DataTable();
